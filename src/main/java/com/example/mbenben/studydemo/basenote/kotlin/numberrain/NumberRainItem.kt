...
class NumberRainItem(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    var textSize = 15 * resources.displayMetrics.density
        set(value) {
            field = value
            //MDove：兼容低版本
            if (ViewCompat.isAttachedToWindow(this@NumberRainItem)) {
                postInvalidate()
            }
        }
    ...
    private fun parseAttrs(attrs: AttributeSet) {
        ...
        normalColor = typedArray.getColor(R.styleable.NumberRainItem_nri_normalColor, Color.GREEN)
        hightLightColor = typedArray.getColor(R.styleable.NumberRainItem_nri_highLightColor, Color.RED)
        startOffset = typedArray.getInt(R.styleable.NumberRainItem_nri_startOffset, 0).toLong()
        textSize = typedArray.getDimension(R.styleable.NumberRainItem_nri_textSize, textSize)
        typedArray.recycle()
    }
    ...
    private fun drawNumbers(canvas: Canvas, count: Int) = if (count == 0) {
